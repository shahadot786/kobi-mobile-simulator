...
struct SimulatorView: View {
    ...
    private var telemetryStatusBar: some View {
        HStack(spacing: 16) {
            ...
            .accessibilityLabel(
                "\(String(localized: "accessibility.telemetry.viewport")): \(viewModel.device.viewportWidth) by \(viewModel.device.viewportHeight)"
            )
            ...
            .accessibilityLabel(viewModel
                .loadError == nil ? String(localized: "simulator.status.online") :
                String(localized: "simulator.status.offline"))
            ...
            if viewModel.networkThrottlePreset != .none {
                ...
                .clipShape(Capsule())
                .accessibilityLabel(String(localized: "accessibility.status.throttle") +
                    ": \(viewModel.networkThrottlePreset.label)")
            }
            ...
            HStack(spacing: 6) {
                Text(viewModel.device.name)
                    .fontWeight(.medium)
                Text(viewModel
                    .orientation == .portrait ? "simulator.orientation.portrait" : "simulator.orientation.landscape")
                    .foregroundStyle(.secondary)
            }
            .font(.caption)
            .accessibilityElement(children: .combine)
            .accessibilityLabel("\(viewModel.device.name), \(orientationDescription)")
        }
        .font(.caption)
    ...
    /// Shared with `telemetryStatusBar`'s accessibility label so the two stay in sync and neither
    /// line exceeds the project's max line length.
    private var orientationDescription: String {
        viewModel.orientation == .portrait
            ? String(localized: "simulator.orientation.portrait")
            : String(localized: "simulator.orientation.landscape")
    }
...
// SimulatorView's primary type body under this project's SwiftLint length limit — same file,
